...
async function writeToDatabase(userStatistics) {
    const baseUrl = process.env.DATABASE_URL;
    const id = userStatistics.id;
    ...
    for (const key in endpoints) {
        if (userStatistics[key] !== undefined) {
            const url = `${baseUrl}/update/${endpoints[key]}`;
            const body = {
                id: id,
                [key]: userStatistics[key],
            };
            ...
            try {
                ...
                throw new Error("Something went wrong trying to upload the statistics to the databse!");
            }
        }
    }
}
